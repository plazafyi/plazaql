...
import { typeCheck, inferChainStateAtPosition } from "./type-checker.js";
...
export interface CompletionItem {
  label: string;
  kind: "method" | "variable" | "keyword" | "tag" | "function" | "param" | "value" | "operator";
  detail?: string;
...
export function getCompletions(
  ...
  switch (context.kind) {
    case "dot":
      return getDotCompletions(source, context.currentType, context.lastGroup, context.lastOrdinal);
    case "dollar":
    ...
    case "inside_method_params":
      if (context.paramName) {
        return getMethodParamValueCompletions(context.methodName, context.paramName);
      }
...
type AnalyzedContext =
  | { kind: "dot"; currentType: PqlType | null; lastGroup: MethodGroup; lastOrdinal: number }
  | { kind: "dollar" }
  | { kind: "dollar_dollar" }
  | { kind: "inside_search"; subContext: "element_type" | "tag_key" | "tag_value"; tagKey?: string }
  | { kind: "inside_filter"; subContext: "tag_key" | "tag_value"; tagKey?: string }
...
function analyzeContext(source: string, line: number, col: number): AnalyzedContext {
  ...
  if (trimmed.endsWith(".") || /\.\w*$/.test(textUpTo)) {
    const { type, lastGroup, lastOrdinal } = inferTypeBeforeDot(source, line, col);
  ...
  if (innermost && innermost in METHOD_PARAM_VALUES) {
    ...
    return { kind: "inside_method_params", methodName: innermost, paramName: paramName ?? undefined };
  }
  if (innermost && innermost in FUNCTION_SIGNATURES) {
    const paramName = detectParamName(textUpTo, innermost);
    return { kind: "inside_function_params", functionName: innermost, paramName: paramName ?? undefined };
  }
...
function analyzeSearchContext(textUpTo: string): AnalyzedContext {
  ...
  if (innerText === null) return { kind: "inside_search", subContext: "tag_key" };
  ...
  if (tagValueMatch) {
    return { kind: "inside_search", subContext: "tag_value", tagKey: tagValueMatch[1] };
  }
...
function analyzeTagFilterContext(textUpTo: string, funcName: string): AnalyzedContext {
  const innerText = extractCallInner(textUpTo, funcName);
  if (innerText === null) return { kind: "inside_filter", subContext: "tag_key" };
  ...
  const tagValueMatch = trimmedSeg.match(/^([\w:]+)\s*:\s*/);
  if (tagValueMatch) {
    return { kind: "inside_filter", subContext: "tag_value", tagKey: tagValueMatch[1] };
  }
...
function getLastSegment(text: string): string {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    else if (text[i] === "," && depth === 0) lastComma = i;
  }
...
function inferTypeBeforeDot(
  ...
  try {
    ...
    if (ast.length === 0) return { type: null, lastGroup: "source", lastOrdinal: 0 };
...
function getDotCompletions(
  ...
  if (currentType && isTerminal(currentType)) return [];
  if (currentType && !isChainable(currentType)) return [];

  const items: CompletionItem[] = [];
  const noArgMethods = new Set(["centroid", "count", "ids", "tags", "skel", "elevation", "area", "length", "first", "last"]);
  ...
  for (const m of METHOD_CATALOG) {
    ...
    if (m.group === "freely_orderable" && lastGroup === "freely_orderable" && lastOrdinal > 0) {
      const mp = methodPhase(m.name);
      if (mp.ordinal < lastOrdinal && mp.ordinal > 0) continue;
    }
...
function getTagCompletions(): CompletionItem[] {
  return OSM_TAG_DATABASE.map((tag) => ({
    ...
    documentation: tag.values.length > 0
      ? `Values: ${tag.values.slice(0, 8).map((v) => v.value).join(", ")}${tag.values.length > 8 ? ", ..." : ""}`
...
function getFilterOperatorCompletions(): CompletionItem[] {
  return [
    { label: "*", kind: "operator", detail: "Key exists (any value)", insertText: "*", sortText: "z1" },
    { label: "!*", kind: "operator", detail: "Key does not exist", insertText: "!*", sortText: "z2" },
    { label: '~"…"', kind: "operator", detail: "Regex match", insertText: '~"', sortText: "z3" },
    { label: '~i"…"', kind: "operator", detail: "Case-insensitive regex", insertText: '~i"', sortText: "z4" },
    { label: '!~"…"', kind: "operator", detail: "Negated regex", insertText: '!~"', sortText: "z5" },
    { label: '!"…"', kind: "operator", detail: "Not equal", insertText: '!"', sortText: "z6" },
  ];
}

function getElementTypeCompletions(): CompletionItem[] {
  return [
    { label: "node", kind: "keyword", detail: "OSM Node — points (shops, POIs, etc.)", insertText: "node, ", sortText: "!a" },
    { label: "way", kind: "keyword", detail: "OSM Way — lines and polygons (roads, buildings)", insertText: "way, ", sortText: "!b" },
    { label: "relation", kind: "keyword", detail: "OSM Relation — grouped elements (routes, boundaries)", insertText: "relation, ", sortText: "!c" },
    { label: "nwr", kind: "keyword", detail: "Any element type (node + way + relation)", insertText: "nwr, ", sortText: "!d" },
  ];
}
...
function getParamValueCompletions(functionName: string, paramName: string): CompletionItem[] {
  const sig = FUNCTION_SIGNATURES[functionName];
  if (!sig) return [];
  const param = sig.params.find((p) => p.name === paramName);
  if (!param?.enumValues) return getParamCompletions(functionName);
...
function getMethodParamValueCompletions(methodName: string, paramName: string): CompletionItem[] {
  ...
  if (!values) return getMethodParamCompletions(methodName);
