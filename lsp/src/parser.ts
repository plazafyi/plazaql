...
import { Parser as TreeSitterParser, Language as TreeSitterLanguage, type Node as SyntaxNode } from "web-tree-sitter";
...
function tryParseSettingsLine(trimmed: string, lineNum: number): SettingsNode | null {
...
function collectErrors(node: SyntaxNode, errors: ParseError[], insideMethod = false): void {
  ...
  for (let i = 0; i < node.childCount; i++) {
    collectErrors(node.child(i)!, errors, enterMethod);
  }
}

function transformProgram(root: SyntaxNode): { ast: Statement[]; errors: ParseError[] } {
  const ast: Statement[] = [];
...
function recoverStatementsFromError(errorNode: SyntaxNode, ast: Statement[]): void {
  ...
  for (let i = 0; i < errorNode.namedChildCount; i++) {
    ...
    if (expr) {
      ...
      if (prevSibling?.type === "output_ref") {
        ...
        ast.push({ kind: "output", name: prevSibling.text.slice(3), expr, pos: pos(prevSibling) });
      } else {
      ...
      for (let j = 0; j < child.namedChildCount; j++) {
        ...
        if (tf.type === "tag_filter") {
          ...
          if (valueNode) {
            ...
            if (val !== null) pairs.push({ key, value: val });
          }
        }
      }
    }
...
function parseDirectiveValue(node: SyntaxNode): string | number | boolean | null {
  switch (node.type) {
    case "number": {
...
function transformIdentifier(node: SyntaxNode): IdentifierNode {
  ...
  const name = node.type === "bare_identifier"
...
function transformMethodCall(node: SyntaxNode): Expr | null {
  ...
  if (!receiverNode || !methodNode) return null;
  ...
  return { kind: "chain", receiver, method, pos: pos(receiverNode) } as ChainNode;
}

function findSearchRoot(expr: Expr): SearchNode | null {
  if (expr.kind === "search") return expr;
...
function transformFilterPrimary(node: SyntaxNode): Expr | null {
  switch (node.type) {
    ...
    case "filter_expression":
      if (node.namedChildCount === 1) {
        return transformFilterPrimary(node.namedChild(0)!);
      }
      return null;
    default:
      // Try as regular expr
...
function transformTagFilter(node: SyntaxNode): TagFilter | null {
  if (node.type === "tag_filter") {
    ...
    if (!valueNode) return { op: "exists", key };
...
function transformTagFilterValue(key: string, node: SyntaxNode): TagFilter {
  switch (node.type) {
    ...
    case "tag_neq":
      return { op: "neq", key, value: unquoteString(node.namedChild(0)?.text ?? "") };
    case "tag_regex":
      return { op: "regex", key, value: unquoteString(node.namedChild(0)?.text ?? "") };
    case "tag_regex_i":
      return { op: "regex_i", key, value: unquoteString(node.namedChild(0)?.text ?? "") };
    case "tag_not_regex":
      return { op: "not_regex", key, value: unquoteString(node.namedChild(0)?.text ?? "") };
    case "tag_exists":
...
function collectComputationArgs(node: SyntaxNode): Arg[] {
  ...
  for (let i = 0; i < node.namedChildCount; i++) {
    const child = node.namedChild(i)!;
    if (child.type === "computation_name") continue;
    if (child.type === "keyword_argument") {
      const kwarg = transformKwarg(child);
      if (kwarg) args.push(kwarg);
    } else {
