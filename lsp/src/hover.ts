...
export function getHover(
  ...
  const elementDescriptions: Record<string, string> = {
    ...
    relation:
      "OSM Relation — a group of nodes/ways with roles. Examples: bus routes, admin boundaries.",
...
function getTokenAtPosition(
  ...
  // Find if cursor is within a $$.name token by scanning backward
  {
    ...
    while (s > 0 && isIdentChar(currentLine[s - 1] as string)) {
      s--
    }
    // Check for preceding "$$."
    if (
  ...
  let start = idx
  while (start > 0 && isIdentChar(currentLine[start - 1] as string)) {
    start--
  }
  let end = idx
  while (end < currentLine.length && isIdentChar(currentLine[end] as string)) {
    end++
  }
...
function findIdentEnd(line: string, start: number): number {
  let end = start
  while (end < line.length && isIdentChar(line[end] as string)) {
    end++
  }
  return end
}
